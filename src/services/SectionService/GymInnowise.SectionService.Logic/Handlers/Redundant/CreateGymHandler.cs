...
{
    public class CreateGymHandler : IRequestHandler<CreateRedundantCommand<Gym>>
    {
        private readonly IRedundantRepository<GymEntity> _gymRepository;
        private readonly IMapper<Gym, GymEntity> _gymMapper;

        public CreateGymHandler(IRedundantRepository<GymEntity> gymRepository, IMapper<Gym, GymEntity> gymMapper)
        {
            _gymRepository = gymRepository;
            _gymMapper = gymMapper;
        }

        public async Task Handle(CreateRedundantCommand<Gym> request, CancellationToken cancellationToken)
        {
            var entity = _gymMapper.Map(request.Data);
